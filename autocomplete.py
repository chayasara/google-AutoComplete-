...
def smaller_strings(matches):
    return sorted(matches, key=lambda i:i['string'])[:5]
...
def create_autocompleteData(matches):
    res = []
    for match in matches:
        res.append(AutoCompleteData(match['string'], match['filename'], match['offset'], match['score']))
    return res
...
def get_perfect_matches(term, i, trie, strings, new_term = None):
    ...
    return [{'id': id, 'offset': calc_offset(new_term, strings[id][0]), 'string': strings[id][0], 'filename': strings[id][1]} for id in cur['ids']]
...
def perfect_matches(term, trie, strings):
    ...
    if len(matches) >= 5:
        ...
        return create_autocompleteData(matches)
...
def replacement_matches(term, trie, index, matches, strings):
    result = matches
    score = replacement_score({'term':term[index:], 'index':index})
    for letter in replacement_letters(trie, term[index:]):#don't want to replace with same letter
...
def add_letter_matches(term, trie, index, matches,strings):
    score = remove_add_letter_score({'term':term[index:], 'index':index})
    result = matches
